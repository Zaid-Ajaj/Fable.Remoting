...
let invoke<'out, 'impl> (funcName: string) (record: 'impl) (input: Choice<obj[], string>) =
    ...
    match proxy { ImplementationBuilder = (fun () -> record); Input = inp; EndpointName = funcName; HttpVerb = "POST"; IsContentBinaryEncoded = false; IsProxyHeaderPresent = true; Output = output } |> Async.RunSynchronously with
